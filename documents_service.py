...
# Model: gemini-3.1-flash-lite-preview supports multimodal (text + vision)
GEMINI_MODEL = "gemini-3.1-flash-lite-preview"

# Image types we handle with inline bytes (fastest path, no File API round-trip)
...
# ─── App ─────────────────────────────────────────────────────────────────────
app = FastAPI(title="AetherDocs Intelligence Service", version="1.2.0")
...
def parse_json_response(raw: str) -> dict:
    """Extract valid JSON from a model response, stripping markdown fences if present."""
    raw = raw.strip()
    # Strip ```json ... ``` or ``` ... ```
...
def build_gemini_contents(file_bytes: bytes, ext: str, temp_file_path: str) -> list:
    """
    Choose the optimal content representation:
    - Images → inline base64 bytes (no File API, fastest)
    - Plain text / CSV → inline text string
    - PDF / others → Gemini File API upload
    """
    lower = ext.lower()
...
async def _notify_interconnect_document(document_id: str, extracted_data: dict) -> None:
    """Fire-and-forget: notify interconnect layer after successful document extraction."""
    try:
        async with httpx.AsyncClient(timeout=10.0) as client:
            await client.post(
                f"{INTERCONNECT_URL}/api/interconnect/document-processed",
                json={"document_id": document_id, "extracted_data": extracted_data},
            )
...
async def process_document(document_id: str, background_tasks: BackgroundTasks):
    ...
    logger.info(f"Processing document {document_id} ({storage_path})")
    ...
    # Write temp file (needed for Gemini File API path; images skip this)
    temp_file_path = None
    extracted_data: dict = {}
    ...
    try:
        ...
        # Build contents for Gemini (images → inline, text → inline, PDF → File API)
        ...
        # Surface quota / permission errors clearly to the caller
    ...
    # Notify interconnect layer in the background (non-blocking)
...
async def get_signed_url(document_id: str, expires_in: int = 3600):
    """Generate a temporary signed URL for previewing the raw file."""
...
async def delete_document(document_id: str):
    """Delete the document from Supabase Storage AND the database."""
    ...
    logger.info(f"Deleting document {document_id} ('{file_name}', storage: {storage_path})")

    # Delete from Supabase Storage (best-effort — don't fail if file is already gone)
    storage_ok = False
    try:
        supabase.storage.from_(STORAGE_BUCKET).remove([storage_path])
        storage_ok = True
        logger.info(f"Storage object '{storage_path}' removed")
    except Exception as e:
    ...
    try:
        ...
        logger.info(f"DB record {document_id} deleted")
    except Exception as e:
...
def build_documents_context(docs: list[dict]) -> str:
    """Serialize all done documents into a readable context string for Gemini."""
    if not docs:
...
async def chat_with_documents(req: ChatRequest):
    """
    Stateless chat endpoint grounded in all processed documents.
    The full conversation history is sent by the client on every request.
    """
    # 1. Fetch all done documents from Supabase
    try:
        ...
        logger.error(f"Supabase fetch failed in chat endpoint: {e}")
    ...
    #    google-genai uses Content objects with role + parts
    history_contents = []
    ...
    try:
        ...
        err_msg = str(e)
        logger.error(f"Gemini chat error: {err_msg}")
        if "429" in err_msg or "RESOURCE_EXHAUSTED" in err_msg:
    ...
    logger.info(f"Chat reply (first 120 chars): {reply[:120]}")
    return {"reply": reply}
...
class InvoiceDraftRequest(_BaseModel):
    prompt: str
...
@app.post("/api/invoice/draft")
async def draft_invoice(req: InvoiceDraftRequest):
    """Use Gemini to convert a natural-language prompt into structured invoice line items."""
    ...
    try:
        ...
        err = str(e)
        logger.error(f"Invoice draft error: {err}")
        if "429" in err or "RESOURCE_EXHAUSTED" in err:
            raise HTTPException(status_code=429, detail="AI quota exceeded. Please try again shortly.")
        raise HTTPException(status_code=500, detail=f"Failed to generate invoice draft: {err}")


# Run locally:
# python -m uvicorn documents_service:app --reload --port 8000
